...
def test_chunk_ranges_1d_slice():
    ...
    # 5:15 -> chunk 0: 5:10, chunk 1: 0:5 (relative to chunk start? No, offsets logic implies absolute indices if using offsets?)
    # Wait, let's re-read _slice_ranges in src/xarray_kat/utils/chunk_selection.py
    # It returns "range of indices in 'offsets' that fall within [start, stop]"
    # But _1d_chunk_ranges logic:
    # return _slice_ranges(offsets, indexer)
    # _slice_ranges returns tuple of pairs of points.
    # The points are absolute (based on offsets).
    # Wait, the docstring says "return the start and stop indices of each chunk that would be accessed by the indexer."
    # But let's check what _slice_ranges returns.
    # It uses bisect on offsets (0, 10, 20).
    # If slice is 5:15.
    # bisect_left(offsets, 5) -> 1 (offsets[1] is 10? No. offsets is [0, 10, 20])
    # bisect_left([0, 10, 20], 5) -> 1.
    # bisect_right([0, 10, 20], 15) -> 2.
    # boundary_points = offsets[1:2] -> [10]
    # start=5, stop=15.
    # insert 5 -> [5, 10]
    # append 15 -> [5, 10, 15]
    # zip -> (5, 10), (10, 15).
...
def test_chunk_ranges_1d_array():
    ...
    # returns list of (src_indices, dest_indices)?
    # Wait, let's look at the code again.
    # results.extend(src_dests)
    # src_dst = np.empty((2, ilen), dtype)
    # src_dst[0, :] = sindexer[i:next_i]  (source indices, i.e., indices in the array being accessed)
    # src_dst[1, :] = argsort[i:next_i]   (destination indices, i.e., indices in the result)
    # So it returns a tuple of 2D arrays.
